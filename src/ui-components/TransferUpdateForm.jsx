...
export default function TransferUpdateForm(props) {
  const {
    id: idProp,
    transfer: transferModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = transferRecord
      ? { ...initialValues, ...transferRecord }
      : initialValues;
    setFrom_address(cleanValues.from_address);
    setTo_address(cleanValues.to_address);
    setSignature(cleanValues.signature);
    setAmount_in_sol(cleanValues.amount_in_sol);
    setErrors({});
  };
  const [transferRecord, setTransferRecord] = React.useState(transferModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(Transfer, idProp)
        : transferModelProp;
      setTransferRecord(record);
    };
    queryData();
  }, [idProp, transferModelProp]);
  React.useEffect(resetStateValues, [transferRecord]);
  const validations = {
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            Transfer.copyOf(transferRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "TransferUpdateForm")}
      {...rest}
    >
      <TextField
        label="From address"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || transferModelProp)}
